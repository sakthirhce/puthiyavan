...
@Component
@Slf4j
public class NiftyOptionBuy935V2Test {
    @Autowired
    ...
    public static final Logger LOGGER = Logger.getLogger(NiftyOptionBuy935V2Test.class.getName());
   // @Scheduled(cron = "${niftyBuy935.schedule.entry}")
    public void buy() throws ParseException, KiteException, IOException {
        sendMessage.sendToTelegram("niftyBuy935 execution started", telegramToken);
        ...
        try {
                    rangeSelected.entrySet().stream().forEach(atmNiftyStrikeMap -> {
                                ...
                                userList.getUser().stream().filter(
                                    ...
                                    user.getNiftyBuy935().getLotConfig().entrySet().forEach(day -> {
                                    ...
                                    try {
                                        ...
                                        //    orderPlacementRequest.setExchangeSegment(DhanExchangeSegment.NSE_FNO.name());
                                         ...
                                         //   workerFactory.placeOrder(orderParams,user,atmNiftyStrikeMap.getValue(),tradeData);
                                        }
                                        ...
                                        user.getNiftyBuy935().straddleTradeMap.put(atmNiftyStrikeMap.getValue().getZerodhaSymbol(), tradeData);
                                       // mapTradeDataToSaveOpenTradeDataEntity(tradeData,true);
                                        ...
                                        sendMessage.sendToTelegram("Error while placing nifty buy order: " + atmNiftyStrikeMap.getKey() + ":" + user.getName() + ",Exception:" + e.getMessage()+":"+getAlgoName()+" Input:"+new Gson().toJson(orderParams), telegramToken);
                            ...
                            if (user.getNiftyBuy935().getLotConfig() != null && user.getNiftyBuy935().buyConfig.isEnabled()) {
                                ...
                                user.getNiftyBuy935().getBuyConfig().getLotConfig().forEach((lotValue, value1) -> {
                                    if (lotValue.contains(todayCaps)) {
                                        ...
                                        positions.stream().filter(position -> "NRML".equals(position.product) && tradeData.getStockName().equals(position.tradingSymbol) && (position.netQuantity != 0)).forEach(position -> {
