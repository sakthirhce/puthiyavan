...
@Component
@Slf4j
public class NiftyVwapRsiOiVolumeBuyBacktest {
    @Autowired
    ...
    public void buy(int day,String tf,String trialPercent,String multi, String slipage,boolean targetEnabled) throws ParseException, KiteException, IOException {
        ...
        while (day<0) {
          ...
          //  System.out.println(nifty);
            ...
            HistoricalData historicalData = zerodhaAccount.kiteSdk.getHistoricalData(dateTimeFormat.parse(currentDate + " 09:15:00"), dateTimeFormat.parse(currentDate + " 15:15:00"), "256265", interval, false, false);
                ...
                historicalData.dataArrayList.stream().forEach(historicalData1 -> {
                    try {
                        ...
                        if (closeDate.equals(currentDate) && openDatetime.after(dateTimeFormatT.parse(openDate + "T09:14:00")) && closeDatetime.before(dateTimeFormatT.parse(closeDate + "T15:00:00")) && (dateMap.get("NEXT_CHECK") == null || (closeDatetime.equals(dateMap.get("NEXT_CHECK"))) || (closeDatetime.after(dateMap.get("NEXT_CHECK"))))) {
                            ...
                            strikeList.add("NIFTYWK" + atmStrike + "CE");
                            strikeList.add("NIFTYWK" + atmStrike + "PE");
                            int iTMPE = atmStrike + 50;
                            strikeList.add("NIFTYWK" + iTMPE + "PE");
                            int iTMCE = atmStrike - 50;
                            strikeList.add("NIFTYWK" + iTMCE + "CE");
                            ...
                            strikeList.stream().forEach(strike -> {
                                try {
                                   ...
                                   if(expDayWithS.contains(String.valueOf(lastThursday.getDayOfMonth()))){
                                       fileName="NIFTY"+strike.substring(7,strike.length()-2)+strike.substring(strike.length()-2);
                                   }else{
                                       fileName=strike;
                                   }
                                    String filePath = "/home/hasvanth/Downloads/NiftyWk/" + year + "_"+tf+"/" + month + "/Expiry " + expDayWithS + " " + month + "/"+tf+"/" + fileName + ".csv";
                                    ...
                                    historicalDataEx.dataArrayList.stream().forEach(historicalDataExtended -> {
                                        try {
                                            ...
                                            if (closeDatetime.equals(closTime)) {
                                                ...
                                                if (rsiCo && volumeCo && vapCo  && (tradeMap.size()== 0 || (tradeMap.get(strike)==null || (tradeMap.get(strike)!=null && tradeMap.get(strike).isExited && !tradeMap.get(strike).isOrderPlaced))) ) {
                                                    ...
                                                    BigDecimal targetPrice = tradeData.getBuyPrice().add(slPoints.multiply(new BigDecimal(multi))).setScale(0, RoundingMode.HALF_UP);
                                                    tradeData.setTargetPrice(targetPrice);
                                            ...
                                            if (tradeMap.get(strike) != null && tradeMap.get(strike).isOrderPlaced && !tradeMap.get(strike).isExited && closTime.after(closeDatetime)) {
                                                ...
                                                if (new BigDecimal(historicalDataExtended.low).compareTo(tradeData.getSlPrice()) < 0) {
                                                    boolean slExit=rsiCo && volumeCo && vapCo && oiCo;
                                                ...
                                                else if (!tradeData.isExited && targetEnabled && new BigDecimal(historicalDataExtended.high).compareTo(tradeData.getTargetPrice())>0) {
                                                    tradeData.isExited = true;
