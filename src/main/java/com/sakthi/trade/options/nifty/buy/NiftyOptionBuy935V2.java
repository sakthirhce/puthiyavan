...
@Component
@Slf4j
public class NiftyOptionBuy935V2 {
    @Autowired
    ...
    public void buy() throws ParseException, KiteException, IOException {
     ...
     //   String nifty = zerodhaTransactionService.niftyIndics.get("NIFTY 50");
       ...
       /* String historicURL = "https://api.kite.trade/instruments/historical/" + nifty + "/5minute?from=" + currentDate + "+09:00:00&to=" + currentDate + "+11:15:00";
        ...
        rangeSelected=mathUtils.getPriceRangeSortedWithLowRangeNifty(currentDate,200,150,"09:34:00","NF");
     /*   executorService1.submit(() -> {
            try {
                Map<String,Map<String,String>> atmStrikesStraddle;
                if (zerodhaTransactionService.expDate.equals(currentDate)) {
                    atmStrikesStraddle = zerodhaTransactionService.niftyNextWeeklyOptions;
                } else {
                    atmStrikesStraddle = zerodhaTransactionService.niftyWeeklyOptions;
                }
        ...
        rangeSelected.forEach((key, value) -> {
            LOGGER.info(key + ":" + new Gson().toJson(value));
            try {
                sendMessage.sendToTelegram("RANGE STRIKE:"+key+":"+new Gson().toJson(value), telegramToken);
            }catch (Exception e){
                log.error("error:"+e);
            }
        });
        ...
        try {
                    rangeSelected.entrySet().stream().forEach(atmNiftyStrikeMap -> {
                                ...
                                userList.getUser().stream().filter(
                                        user -> user.getNiftyBuy935V2() != null && user.getNiftyBuy935V2().isNrmlEnabled() && user.getNiftyBuy935V2().getLotConfig().containsKey(todayCaps)
                                ).forEach(user -> {
                                    BrokerWorker brokerWorker = workerFactory.getWorker(user);
                                    AtomicInteger qty = new AtomicInteger(1);
                                    user.getNiftyBuy935V2().getLotConfig().entrySet().forEach(day -> {
                                    ...
                                    if (user.getNiftyBuy935V2().getLotConfig() != null && user.getNiftyBuy935V2().buyConfig.isEnabled()) {
                                        user.getNiftyBuy935V2().getBuyConfig().getLotConfig().forEach((lotValue, value1) -> {
                                            if (lotValue.contains(todayCaps)) {
                                    ...
                                    try {
                                        ...
                                        user.getNiftyBuy935V2().straddleTradeMap.put(atmNiftyStrikeMap.getValue().getDhanSymbol(), tradeData);
                                       // mapTradeDataToSaveOpenTradeDataEntity(tradeData,true);
                                        String message="option buy limit order placed for for user:" + user.getName() + " strike: " + atmNiftyStrikeMap.getKey()+":"+getAlgoName()+":broker:"+user.getBroker();
                                        LOGGER.info(message);
                                        try {
                                        ...
                                        LOGGER.info("Error while placing nifty buy order: "+ atmNiftyStrikeMap.getKey()+":" + e.getMessage()+getAlgoName());
                                        sendMessage.sendToTelegram("Error while placing nifty buy order: " + atmNiftyStrikeMap.getKey() + ":" + user.getName() + ",Exception:"+getAlgoName()+" Input:"+new Gson().toJson(orderParams), telegramToken);

                                    } catch (KiteException e) {
                                        throw new RuntimeException(e);
                                    }
                                    LOGGER.info(new Gson().toJson(user.getNiftyBuy935V2().straddleTradeMap));
                                });
                            });
   ...
   // @Scheduled(cron = "${banknifty.intraday.buy.exit.schedule}")
        ...
        userList.getUser().stream().filter(user -> user.getNiftyBuy935V2() != null && user.getNiftyBuy935V2().isNrmlEnabled()).forEach(user -> {
            try {
               ...
               BrokerWorker brokerWorker = workerFactory.getWorker(user);
                    ...
                    positions=brokerWorker.getPositions(user);

                //LOGGER.info(new Gson().toJson(positions));
                if (user.getNiftyBuy935V2().straddleTradeMap != null) {
                    user.getNiftyBuy935V2().straddleTradeMap.entrySet().stream().filter(map -> map.getValue().isOrderPlaced
                            && map.getValue().getEntryOrderId() != null && !map.getValue().isExited).forEach(straddleTradeMap -> {
                        TradeData tradeData = straddleTradeMap.getValue();
                        log.info("nifty 15:10 exit started 1");
                        if (user.getName().equals(straddleTradeMap.getValue().getUserId())) {
                            if (user.getNiftyBuy935V2().getLotConfig() != null && user.getNiftyBuy935V2().buyConfig.isEnabled()) {
                                log.info("nifty 15:10 exit started 2");
                                LocalDate localDate = LocalDate.now();
                                DayOfWeek dow = localDate.getDayOfWeek();
                                String today = dow.getDisplayName(TextStyle.SHORT_STANDALONE, Locale.ENGLISH);
                                String todayCaps = today.toUpperCase();
                                ...
                                user.getNiftyBuy935V2().getBuyConfig().getLotConfig().forEach((lotValue, value1) -> {
                                    if (lotValue.contains(todayCaps)) {
                                        ...
                                        qty.getAndSet(value);
                                        log.info("nifty 15:10 exit started 3");
                                        orders.stream().filter(order -> ("PENDING".equals(order.status) || "OPEN".equals(order.status) || "TRIGGER PENDING".equals(order.status)) && order.orderId.equals(tradeData.getSlOrderId())).forEach(orderr -> {
                                            try {
                                                // if(straddleTradeMap.getKey().contains("BUY")) {
                                                OrderParams orderParams = new OrderParams();
                                                log.info("nifty 15:10 exit started 4");
                                        ...
                                        positions.stream().filter(position -> ("NRML".equals(position.product)||"MARGIN".equals(position.product)) && (tradeData.getStockName().equals(position.tradingSymbol)||tradeData.getStrikeId().equals(position.tradingSymbol)) && (position.netQuantity != 0)).forEach(position -> {
                                            ...
                                            if (position.netQuantity > 0) {
                                                ...
                                                try {
                                                    ...
                                                    user.getNiftyBuy935V2().straddleTradeMap.put(tradeDataBuy.getStockName() + "-BUY-INTRADAY", tradeDataBuy);
    ...
    public void sLMonitorScheduler() {
        ...
        userList.getUser().stream().filter(user -> user.getNiftyBuy935V2() != null && user.getNiftyBuy935V2().isNrmlEnabled()).forEach(user -> {
           BrokerWorker brokerWorker= workerFactory.getWorker(user);
            if (user.getNiftyBuy935V2().straddleTradeMap != null) {
                user.getNiftyBuy935V2().straddleTradeMap.entrySet().stream().filter(map -> map.getValue().isOrderPlaced && map.getValue().getEntryOrderId() != null)
                        .forEach(map -> {
                            try {
                                ...
                                orderList.forEach(order -> {
                                    if (!trendTradeData.isErrored && !trendTradeData.isSLCancelled && !trendTradeData.isExited && !trendTradeData.isSLHit) {
                                                ...
                                                if ("BUY".equals(order.transactionType)) {
                                                    ...
                                                    try {
                                                        ...
                                                        try {
                                                            ...
                                                            String message = MessageFormat.format("Option Buy Triggered for {0}", trendTradeData.getStockName() + ":" + user.getName() + ":" + getAlgoName()+":broker:"+user.getBroker());
                                                            LOGGER.info(message);
                                                            sendMessage.sendToTelegram(message, telegramToken);
                                                        } catch (Exception e) {
                                                            e.printStackTrace();
                                                        ...
                                                        try {
                                                            ...
                                                            try {
                                                                ...
                                                                sendMessage.sendToTelegram("Nifty option : " + trendTradeData.getStockName() + ":" + user.getName() + " bought and placed SL" + ":" + getAlgoName()+":broker:"+user.getBroker(), telegramToken);
                                                            } catch (Exception e) {
                                                                LOGGER.info("error:" + e);
                                                            }
                                                            ...
                                                            sendMessage.sendToTelegram("Nifty option order: " + trendTradeData.getStockName() + ":" + user.getName() + ": Status: " + order.status + ": error message:" + order.statusMessage + ":" + getAlgoName()+":broker:"+user.getBroker(), telegramToken);
                                                            //e.printStackTrace();
                                                        }
                                        ...
                                        if (("COMPLETE".equals(order.status) || "TRADED".equals(order.status)) && "SELL".equals(order.transactionType) && order.orderId.equals(trendTradeData.getSlOrderId()) && !trendTradeData.isExited && trendTradeData.isSlPlaced) {
                                            ...
                                            String message = MessageFormat.format("SL Hit for {0}" + ": sl sell slipage" + slipage.toString(), map.getKey() + ":" + user.getName() + ":" + getAlgoName()+":broker:"+user.getBroker());
                                            LOGGER.info(message);
                                            sendMessage.sendToTelegram(message, telegramToken);
                                        ...
                                        String message = MessageFormat.format("SL order placement rejected for {0}", map.getKey() + ":" + user.getName() + ":" + order.status + ":" + order.statusMessage + ":" + getAlgoName()+":broker:"+user.getBroker());
                                        LOGGER.info(message);
