...
@Component
@Slf4j
public class BankNiftyShortStraddleOIDBBuyBacktest {
        ...
        list.dataArrayList.stream().forEach(tickData -> {
            try {
             ...
             if (tickDateTime.equals(sdf.parse(tradeTime)) && !shortTradeData.isOrderPlaced ) {
                 ...
                 BigDecimal slTPrice = (new BigDecimal(tickData.close).divide(new BigDecimal(100))).multiply(new BigDecimal(25)).setScale(0, RoundingMode.HALF_UP);
                 BigDecimal price = ((new BigDecimal(tickData.close)).subtract((new BigDecimal(tickData.close).divide(new BigDecimal(100))).multiply(new BigDecimal(25)))).setScale(0, RoundingMode.HALF_UP);
                 BigDecimal target = ((new BigDecimal(tickData.close)).add((new BigDecimal(tickData.close).divide(new BigDecimal(100))).multiply(new BigDecimal(50)))).setScale(0, RoundingMode.HALF_UP);
                 shortTradeData.setTargetPrice(target);
                 shortTradeData.setSlPrice(price);
                 shortTradeData.setSlTrialPoints(slTPrice);
             }
            /* else if (tickDateTime.after(sdf.parse(tradeTime)) && shortTradeData.isOrderPlaced && !shortTradeData.isExited && new BigDecimal(tickData.high).compareTo(shortTradeData.getTargetPrice()) > 0) {
                 shortTradeData.setSellPrice(shortTradeData.getTargetPrice());
                 ...
                 shortTradeData.isExited = true;
                 shortTradeData.setSellTime(strTickDateTime);
                    String[] data = {shortTradeData.getStockName(), shortTradeData.getBuyTime(), shortTradeData.getBuyPrice().toString(), shortTradeData.getSellTime(), shortTradeData.getSellPrice().toString(), shortTradeData.getSellPrice().subtract(shortTradeData.getBuyPrice()).multiply(new BigDecimal(shortTradeData.getQty())).setScale(0, RoundingMode.HALF_UP).toString(), evaluateDate,shortTradeData.getEntryType()};
                    csvWriter.writeNext(data);
                    csvWriter.flush();
