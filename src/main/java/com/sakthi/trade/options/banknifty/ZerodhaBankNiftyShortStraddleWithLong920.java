...
@Component
@Slf4j
public class ZerodhaBankNiftyShortStraddleWithLong920 {
   ...
   // AtomicInteger doubleTopCount = new AtomicInteger(0);
    ...
    public String getAlgoName() {
        return "STRADDLE_LONG_920";
    }

    SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
   // @Scheduled(cron = "${straddle.banknifty.run1.entry}")
        ...
        if (!status.equals("error")) {
            ...
            historicalData.dataArrayList.forEach(historicalData1 -> {
                try {
                    ...
                    if (sdf.format(openDatetime).equals(openDate + "T09:20:00")) {
                        ...
                        atmStrikesStraddle.entrySet().stream().filter(atmStrikeStraddle -> atmStrikeStraddle.getKey().contains(String.valueOf(atmStrike))).forEach(atmBankStrikeMap -> {
                            executorService.submit(() -> {
                                ...
                                userList.getUser().stream().filter(
                                        user -> user.getStraddleConfig920() !=null && user.getStraddleConfig920().isNrmlEnabled() && user.getStraddleConfig920().getLotConfig().containsKey(todayCaps)
                                ).forEach(user -> {
                                    AtomicInteger qty = new AtomicInteger(1);
                                    user.getStraddleConfig920().getLotConfig().entrySet().forEach(day -> {
                                    ...
                                    user.getStraddleConfig920().straddleTradeMap.put(atmBankStrikeMap.getKey(), tradeData);
                                });
                            });
                        });
   ...
   // @Scheduled(cron = "${straddle.banknifty.run1.sl}")
    public void sLMonitorSchedulerImmediate() {
        userList.getUser().stream().filter(user -> user.getStraddleConfig920()!=null && user.getStraddleConfig920().isNrmlEnabled()).forEach(user -> {
            ...
            if (user.getStraddleConfig920().straddleTradeMap != null) {
                user.getStraddleConfig920().straddleTradeMap.entrySet().stream().filter(map -> map.getValue().isOrderPlaced && map.getValue().getEntryOrderId() != null)
                        .forEach(map -> {
                            ...
                            orderList.forEach(order -> {
                                if (trendTradeData.getEntryOrderId().equals(order.orderId) && "SELL".equals(order.transactionType)) {
                                    if( !trendTradeData.isSlPlaced && "COMPLETE".equals(order.status)) {
                                        ...
                                        if (user.getStraddleConfig920().getBuyConfig() != null && user.getStraddleConfig920().getBuyConfig().isEnabled()) {
                                            user.getStraddleConfig920().getBuyConfig().getLotConfig().forEach((lotValue, value1) -> {
                                                if (lotValue.contains(todayCaps)) {
                                                    int value = (Integer.parseInt(value1) * 25) + Integer.valueOf(order.filledQuantity);
                                                    qty.getAndSet(value);
                                                }
                                            });
                                        }
                                        ...
                                        orderParams.product = "NRML";
                                        BigDecimal price = ((triggerPrice.divide(new BigDecimal(100))).add(triggerPrice)).setScale(0, RoundingMode.HALF_UP);
    ...
    //@Scheduled(cron = "${straddle.banknifty.run1.monitor}")
                            ...
                            orderList.forEach(order -> {
                                if (trendTradeData.getEntryOrderId().equals(order.orderId) && !trendTradeData.isSlPlaced && "COMPLETE".equals(order.status) && "SELL".equals(order.transactionType)) {
                                    ...
                                    if (user.getStraddleConfig920().getBuyConfig() != null && user.getStraddleConfig920().getBuyConfig().isEnabled()) {
                                        user.getStraddleConfig920().getBuyConfig().getLotConfig().forEach((lotValue, value1) -> {
                                            if (lotValue.contains(todayCaps)) {
                                                int value = (Integer.parseInt(value1) * 25) + Integer.parseInt(order.filledQuantity);
                                                qty.getAndSet(value);
                                            }
                                        });
                                    }
                                ...
                                else if (order.orderId.equals(trendTradeData.getSlOrderId()) && "BUY".equals(order.transactionType) && trendTradeData.isSlPlaced && !trendTradeData.isErrored &&!trendTradeData.isSLCancelled && !trendTradeData.isExited && !trendTradeData.isSLHit) {
                                    ...
                                    if ("COMPLETE".equals(order.status)) {
                                        if ("BUY".equals(order.transactionType)) {
                                            ...
                                            if (user.getStraddleConfig920().buyConfig.isEnabled()) {
                                                ...
                                                if (qty.get()>0 && user.getStraddleConfig920().getBuyConfig() != null && user.getStraddleConfig920().getBuyConfig().isEnabled()) {
                                                    OrderParams orderParams = new OrderParams();
                                                    orderParams.tradingsymbol = trendTradeData.getStockName();
                                                    orderParams.exchange = "NFO";
                                                    ...
                                                    try {
                                                        ...
                                                        user.getStraddleConfig920().straddleTradeMap.put(trendTradeData.getStockName() + "-BUY", doubleToptradeData);
...
/*
    ...
    @Scheduled(cron = "${straddle.banknifty.run1.nextday.sl.monitor}")
    public void sLMonitorPositions() {
        userList.getUser().stream().filter(user -> user.getStraddleConfig920().isNrmlEnabled()).forEach(user -> {
            if (user.getStraddleConfig920().straddleTradeMap != null) {
                user.getStraddleConfig920().straddleTradeMap.entrySet().stream().filter(map -> map.getValue().isOrderPlaced && map.getValue().getEntryOrderId() != null && !map.getValue().isExited)
                        .forEach(openTradeData -> {
                            ...
                            if (sLOrder.isPresent()) {
                                ...
                                if (order.orderId.equals(openTradeDataEntity.getSlOrderId())){
                                     if(("OPEN".equals(order.status) || "TRIGGER PENDING".equals(order.status))  && !openTradeDataEntity.isExited){
                                         System.out.println("inside open sl cancellation method");
                                        positions.stream().filter(position -> openTradeDataEntity.getStockName().equals(position.tradingSymbol) && !openTradeDataEntity.isExited && position.netQuantity == 0).findFirst().ifPresent(
                                                position -> {
                                                    try {
                                                        user.getKiteConnect().cancelOrder(order.orderId, "regular");
                                                    } catch (KiteException | IOException e) {
...
*/
    ...
    public void pLAndSlippageCalculation(){
        userList.getUser().stream().filter(user -> user.getStraddleConfig920().isNrmlEnabled()).forEach(user -> {
            ...
            if (user.getStraddleConfig920().straddleTradeMap != null) {
                user.getStraddleConfig920().straddleTradeMap.entrySet().stream().filter(map -> map.getValue().isExited)
                        .forEach(openTradeData -> {
   ...
   // @Scheduled(cron = "${straddle.banknifty.run1.nextday.position.load}")
    ...
    //@Scheduled(cron = "${straddle.banknifty.run1.nextday.exit.price}")
  ...
  //  @Scheduled(cron = "${straddle.banknifty.run1.nextday.sl.schedule}")
