...
@Component
@Slf4j
public class BankNiftyOiSellingBackTest {
    @Autowired
            ...
            if (historicalData.dataArrayList.size() > 0) {
                ...
                historicalData.dataArrayList.stream().forEach(historicalData1 -> {
                    try {
                        ...
                        if (closeDate.equals(currentDate) && openDatetime.equals(dateTimeFormatT.parse(openDate + "T09:15:00"))) {
                            ...
                            Map<String, Long> oiCumMap = new HashMap<>();
                            strikeList.stream().forEach(strike -> {
                                try {
                                    ...
                                    String filePath = "/home/hasvanth/Downloads/BankNiftyWk/" + year + "_"+tf+"/" + month + "/Expiry " + expDayWithS + " " + month + "/" + fileName + ".csv";
                                    ...
                                    try {
                                        ...
                                        e.printStackTrace();
                                        if(strike.contains("CE")) {
                                            fileName = "BANKNIFTY" + atmStrike + "CE";
                                        }
                                        else
                                        {
                                            fileName = "BANKNIFTY" + atmStrike + "PE";
                                        }
                                        filePath = "/home/hasvanth/Downloads/BankNiftyWk/" + year + "_"+tf+"/" + month + "/Expiry " + expDayWithS + " " + month + "/" + fileName + ".csv";
                                        csvReader = new CSVReader(new FileReader(filePath));
                                    }
                                    HistoricalDataExtended historicalDataEx = map1MinCSVtoHistoric(csvReader);
                                    Map<String, Long> oidiff = new HashMap<>();
                                    historicalDataEx.dataArrayList.stream().forEach(historicalDataExtended -> {
                                        try {
                                            ...
                                            if (closTime.equals(dateTimeFormat.parse(openDate + " 09:15:00")) && tradeDatat == null) {
                                                oidiff.put("starting_oi", historicalDataExtended.oi);

                                            }
                                            if (closTime.equals(dateTimeFormat.parse(openDate + " 09:59:00")) && tradeDatat == null) {
                                                Long oidif =  historicalDataExtended.oi-oidiff.get("starting_oi");
                                                oiCumMap.put(strike, oidif);
                            ...
                            if (oiCumMap.get("BANKNIFTYWK" + atmStrike + "CE")==0){
                                strikeList.remove("BANKNIFTYWK" + atmStrike + "PE");
                                strikeList.remove("BANKNIFTYWK" + atmStrike + "CE");
                            }

                            if(oiCumMap.get("BANKNIFTYWK" + atmStrike + "CE") > oiCumMap.get("BANKNIFTYWK" + atmStrike + "PE")) {
                                strikeList.remove("BANKNIFTYWK" + atmStrike + "PE");
                            } else {
                                strikeList.remove("BANKNIFTYWK" + atmStrike + "CE");
                            }
                                ...
                                try {
                                    ...
                                    historicalDataEx.dataArrayList.stream().forEach(historicalDataExtended -> {
                                        try {
                                            Date closTime = dateTimeFormat.parse(historicalDataExtended.timeStamp);
                                            TradeData tradeDatat = tradeMap.get(strike);
                                            if (closTime.after(dateTimeFormat.parse(openDate + " 09:59:00")) && tradeDatat == null) {
                                            ...
                                            if (tradeMap.get(strike) != null && tradeMap.get(strike).isOrderPlaced && !tradeMap.get(strike).isExited) {
                                                ...
                                                if (!tradeData.isExited && closTime.equals(dateTimeFormat.parse(dateFormat.format(closTime)+endTime))) {
                                                    ...
                                                    CSVWriter csvWriter = new CSVWriter(new FileWriter(trendPath + "/BankNifty_Backtest/vwap_bank_nifty_vwap_sell_back_test_"+trialPercent+"_pt"+multi+"_oi.csv", true));
...
//                                                }
    ...
    public HistoricalDataExtended map1MinCSVtoHistoric(CSVReader csvReader) {
        String[] lineS;
        HistoricalDataExtended historicalDataOp = new HistoricalDataExtended();
        try {
            int i=0;
            while ((lineS = csvReader.readNext()) != null) {
                if(i>0){
                    HistoricalDataExtended historicalData = new HistoricalDataExtended();
                    historicalData.timeStamp = dateFormat.format(dateTimeFormatForwordSlash.parse(lineS[1]))+" "+lineS[2]+":00";
                    historicalData.open = Double.valueOf(lineS[3]);
                    historicalData.high = Double.valueOf(lineS[4]);
                    historicalData.low = Double.valueOf(lineS[5]);
                    historicalData.close = Double.valueOf(lineS[6]);
                    historicalData.volume = Long.parseLong(lineS[7]);
                    if (lineS.length > 7 && lineS[8]!=null && lineS[8].length() > 0 && lineS[8]!="") {
                        historicalData.oi = new Double(lineS[8]).longValue();
                    }
                    historicalDataOp.dataArrayList.add(historicalData);
                }
                i++;
    ...
    public void cumulativeOI(List<HistoricalDataExtended> historicalDataExtendeds) {
        int i = 0;
        double pre_cumulativeTotal = 0;
        double pre_volume = 0;
        while (i < historicalDataExtendeds.size()) {
