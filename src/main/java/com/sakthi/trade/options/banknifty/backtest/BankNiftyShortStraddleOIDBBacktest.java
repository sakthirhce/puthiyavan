...
@Component
@Slf4j
public class BankNiftyShortStraddleOIDBBacktest {
    ...
    public void triggerNIFTYBANK(LocalDate tradeDate) throws CsvValidationException, ParseException, IOException {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
        Date date = new Date();
        String currentDate = formatter.format(tradeDate);
        ...
        if (!status.equals("error")) {
            try {
                ...
                atm = atmAtomic.get();
                HistoricalDataExtended ceHistoric = new HistoricalDataExtended();
                HistoricalDataExtended peHistoric = new HistoricalDataExtended();
                List<String> atmList = new ArrayList<>();

                getWeeklyExpiryOptionsDetailsBackTest(formatter.format(tradeDate));
                atmList.add(atmAtomic.get() + "CE");
                atmList.add(atmAtomic.get() + "PE");
                OptionOIData optionOIData = new OptionOIData();
                atmList.stream().forEach(atmStrike -> {

                    try {
                        BankNiftyOptionEntity bankNiftyOptionEntity = bankNiftyOptionRepository.findExpStrike(format.parse(expDate), atmStrike);
                        if (bankNiftyOptionEntity != null && bankNiftyOptionEntity.getExpKey() != null && !bankNiftyOptionEntity.getExpKey().isEmpty()) {
                            List<BankNiftyOptionDataEntity> bankNiftyOptionDataEntityList = bankNiftyOptionDataRepository.findExpOptionData(bankNiftyOptionEntity.getExpKey());
                            if (bankNiftyOptionDataEntityList.size() > 0) {
                                if (atmStrike.contains("CE")) {
                                    convertStringToHistoric(atmStrike, ceHistoric, bankNiftyOptionDataEntityList, formatter.format(tradeDate), optionOIData);
                                } else {
                                    convertStringToHistoric(atmStrike, peHistoric, bankNiftyOptionDataEntityList, formatter.format(tradeDate), optionOIData);
                                }
                            }
                        }
                ...
                mapOptionIOData(ceHistoric, peHistoric, optionOIData, formatter.format(tradeDate));
                ...
                if (optionOIData.isTrade) {
                    if (optionOIData.isTradeCE) {
                        ...
                        trade(atmAtomic.get() + "CE", ceHistoric, 25, optionOIData.getTradeTime());

                    } else if (optionOIData.isTradePE) {
                        log.info("PE oi is more than PE,CE OI:" + optionOIData.ceOI3 + "PE Oi:" + optionOIData.peOI3);
                        trade(atmAtomic.get() + "PE", peHistoric, 25, optionOIData.getTradeTime());
                    }
                }
    ...
    public OptionCandleChartData  straddleOI(LocalDate tradeDate,LocalDate expDate,String strikeType){
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
        String currentDate = formatter.format(tradeDate);
        String niftyBank = zerodhaTransactionService.niftyIndics.get("NIFTY BANK");
        String historicURL = "https://api.kite.trade/instruments/historical/" + niftyBank + "/5minute?from=" + currentDate + "+09:00:00&to=" + currentDate + "+15:30:00";
        String response = transactionService.callAPI(transactionService.createZerodhaGetRequest(historicURL));
        System.out.print(response);
        HistoricalData historicalData = new HistoricalData();
        JSONObject json = new JSONObject(response);
        ...
        if (!status.equals("error")) {
            try {
                ...
                atmList.stream().forEach(atmStrike -> {
                    ...
                    try {
                        BankNiftyOptionEntity bankNiftyOptionEntity = bankNiftyOptionRepository.findExpStrike(java.sql.Date.valueOf(expDate), atmStrike);
                        if (bankNiftyOptionEntity != null && bankNiftyOptionEntity.getExpKey() != null && !bankNiftyOptionEntity.getExpKey().isEmpty()) {
                            List<BankNiftyOptionDataEntity> bankNiftyOptionDataEntityList = bankNiftyOptionDataRepository.findExpOptionData(bankNiftyOptionEntity.getExpKey());
                            if (bankNiftyOptionDataEntityList.size() > 0) {
                                if (atmStrike.contains("CE")) {
                ...
                if (optionOIData.isTrade) {
                    if (optionOIData.isTradeCE) {
                        log.info("CE oi is more than PE,CE OI:" + optionOIData.ceOI3 + "PE Oi:" + optionOIData.peOI3);
                        HistoricalDataExtended ceHistoric1=CommonUtil.to_larger_timeframe(ceHistoric,3,formatter.format(tradeDate));
                       return convertHistoricToCandleChartData(ceHistoric1,historicalData,currentDate);

                    } else if (optionOIData.isTradePE) {
                        log.info("PE oi is more than CE,CE OI:" + optionOIData.ceOI3 + "PE Oi:" + optionOIData.peOI3);
                        HistoricalDataExtended peHistoric1=CommonUtil.to_larger_timeframe(peHistoric,3,formatter.format(tradeDate));
                        return convertHistoricToCandleChartData(peHistoric1,historicalData,currentDate);
                    }
                }
                // System.out.println(new Gson().toJson(historicalDataExtendedList));
            } catch (Exception e) {
    ...
    public void trade(String atmStrike, HistoricalDataExtended list,double slPerc,String tradeTime) throws IOException {

        TradeData shortTradeData = new TradeData();
        log.info("Short straddle process started");
        CSVWriter csvWriter = new CSVWriter(new FileWriter(trendPath + "/BankNifty_Backtest/options_straddle_short_new_oi.csv", true));

        log.info("straddle:" + atmStrike);

        SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");
        list.dataArrayList.stream().forEach(tickData -> {
            try {
                ...
                String closeTime = evaluateDate + " 15:09:00";
             ...
             if (tickDateTime.equals(sdf.parse(tradeTime)) && !shortTradeData.isOrderPlaced ) {
                 ...
                 shortTradeData.setSellPrice(new BigDecimal(tickData.close));
                 shortTradeData.isOrderPlaced=true;
                 BigDecimal price = ((new BigDecimal(tickData.close)).add(new BigDecimal(tickData.close).divide(new BigDecimal(4)))).setScale(0, RoundingMode.HALF_UP);
                 shortTradeData.setSlPrice(price);

                } else if (tickDateTime.after(sdf.parse(tradeTime)) && shortTradeData.isOrderPlaced && !shortTradeData.isExited && new BigDecimal(tickData.high).compareTo(shortTradeData.getSlPrice()) > 0) {
    ...
    public void convertStringToHistoric(String strike,HistoricalDataExtended historicalDataExtended,List<BankNiftyOptionDataEntity> bankNiftyOptionDataEntityList,String tradeDate, OptionOIData optionOIData) throws IOException, ParseException {
        bankNiftyOptionDataEntityList.stream().forEach(bankNiftyOptionDataEntity ->
        {
            SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            HistoricalDataExtended historicalDataExtendedL = new HistoricalDataExtended();
            historicalDataExtendedL.setOpen(Double.valueOf(bankNiftyOptionDataEntity.getOpen()));
            historicalDataExtendedL.setHigh(Double.valueOf(bankNiftyOptionDataEntity.getHigh()));
            historicalDataExtendedL.setLow(Double.valueOf(bankNiftyOptionDataEntity.getLow()));
            historicalDataExtendedL.setClose(Double.valueOf(bankNiftyOptionDataEntity.getClose()));
            historicalDataExtendedL.setVolume(Integer.valueOf(bankNiftyOptionDataEntity.getVolume()));
            historicalDataExtendedL.setOi(Long.valueOf(bankNiftyOptionDataEntity.getOi()));
            historicalDataExtendedL.setTimeStamp(sdf1.format(bankNiftyOptionDataEntity.getTradeTime()));

            historicalDataExtended.dataArrayList.add(historicalDataExtendedL);
            });
    }
    public OptionCandleChartData convertHistoricToCandleChartData(HistoricalDataExtended historicalDataExtended,HistoricalData historicalData,String tradeDate) throws Exception, ParseException {
        OptionCandleChartData optionCandleChartData=new OptionCandleChartData();
        List<OptionCandleData> optionCandleDataList=new ArrayList<>();
        List<OptionCandleData> optionCandleDataList1=new ArrayList<>();
    //    StringBuffer sb=new StringBuffer("{ \"name\": \"candle\",\"data\": [");
        AtomicInteger atomicInteger=new AtomicInteger(0);
        historicalDataExtended.dataArrayList.stream().forEach(historicalDataExtendedL ->
        {
            SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");
            OptionCandleData optionCandleData=new OptionCandleData();

            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            try {
                Date datee=sdf.parse(historicalDataExtendedL.timeStamp);

                if(sdf1.format(datee).equals(tradeDate)) {
                    optionCandleData.setX(datee.getTime());

                      List<Double> yaxisData=new ArrayList<>();
                    yaxisData.add(0,historicalDataExtendedL.open);
                    yaxisData.add(1,historicalDataExtendedL.high);
                    yaxisData.add(2,historicalDataExtendedL.low);
                    yaxisData.add(3,historicalDataExtendedL.close);
                    optionCandleData.setY(yaxisData);
                    optionCandleDataList.add(optionCandleData);
                    atomicInteger.getAndAdd(1);
//                    if(1==atomicInteger.get()){
//                        sb.append(" { \"x\": " + datee.getTime() + ", \"y\": [" + historicalDataExtendedL.open + "," + historicalDataExtendedL.high + "," + historicalDataExtendedL.low + "," + historicalDataExtendedL.close + "]}");
//
//                    }else {
//                        sb.append(", { \"x\": " + datee.getTime()  + ", \"y\": [" + historicalDataExtendedL.open + "," + historicalDataExtendedL.high + "," + historicalDataExtendedL.low + "," + historicalDataExtendedL.close + "]}");
//
//                    }
        ...
        historicalData.dataArrayList.stream().forEach(historicalDataExtendedL ->
        {
            SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");
            OptionCandleData optionCandleData=new OptionCandleData();
            ...
            try {
                ...
                if(sdf1.format(datee).equals(tradeDate)) {
                    ...
                    yaxisData.add(3,historicalDataExtendedL.close);
                    optionCandleData.setY(yaxisData);
                    optionCandleDataList1.add(optionCandleData);
                    atomicInteger.getAndAdd(1);
//                    if(1==atomicInteger.get()){
//                        sb.append(" { \"x\": " + datee.getTime() + ", \"y\": [" + historicalDataExtendedL.open + "," + historicalDataExtendedL.high + "," + historicalDataExtendedL.low + "," + historicalDataExtendedL.close + "]}");
//
...
//                    }
      ...
      //  System.out.println(sb.toString());
       // return sb.toString();
        optionCandleChartData.setData(optionCandleDataList);
        optionCandleChartData.setData1(optionCandleDataList1);
       return optionCandleChartData;
    }
    public Map<String,String> lsHoliday=new HashMap<>();

    public OptionOIData mapOptionIOData(HistoricalDataExtended ceList,HistoricalDataExtended peList, OptionOIData optionOIData,String currentDate) throws ParseException {
        AtomicLong ceInitialOi=new AtomicLong();
        AtomicLong peInitialOi=new AtomicLong();
        ceList.dataArrayList.stream().forEach( ceData ->{
            try {

                if(sdf.parse(ceData.timeStamp).equals(sdf.parse(currentDate+" 09:15:00"))){
                    ceInitialOi.getAndAdd(ceData.oi);
                    peList.dataArrayList.stream().filter(peData -> peData.getTimeStamp().equals(ceData.getTimeStamp())).findFirst().ifPresent(peData -> {
                        peInitialOi.getAndAdd(peData.oi);
                    });
                }
            if(sdf.parse(ceData.timeStamp).after(sdf.parse(currentDate+" 09:28:00"))){
                    ...
                    if (sdf.parse(ceData.timeStamp).after(sdf.parse(currentDate + " 09:34:00")) && !optionOIData.isTrade) {
