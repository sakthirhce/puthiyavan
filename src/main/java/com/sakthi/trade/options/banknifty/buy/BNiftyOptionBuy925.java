...
@Component
@Slf4j
public class BNiftyOptionBuy925 {
    @Autowired
  ...
  //  @Scheduled(cron = "${bniftyBuy925.schedule.entry}")
    public void buy() throws ParseException, KiteException, IOException {
        ...
        rangeSelected=mathUtils.getPriceRangeSortedWithLowRange(currentDate,450,350,"09:24:00","BNF");
        ...
        try {
                    rangeSelected.entrySet().stream().forEach(atmNiftyStrikeMap -> {
                                    ...
                                    if (!responseStatus.equals("error")) {
                                        ...
                                        historicalPriceData.dataArrayList.forEach(historicalDataPrice -> {
                                            try {
                                                ...
                                                if (sdf.format(priceDatetime).equals(priceDate + "T09:24:00")) {
                                                   ...
                                                   // BigDecimal triggerPriceTemp = (new BigDecimal(10).add(new BigDecimal(historicalDataPrice.close))).setScale(0, RoundingMode.HALF_UP);
                                                    ...
                                                    LOGGER.info("buy trigger price based on 9:24 close :" + atmNiftyStrikeMap.getKey() + ":" + triggerPriceTemp);
                                                }else {
                                ...
                                userList.getUser().stream().filter(
                                        user -> user.getBniftyBuy925() != null && user.getBniftyBuy925().isNrmlEnabled() && user.getBniftyBuy925().getLotConfig().containsKey(todayCaps)
                                ).forEach(user -> {
                                    BrokerWorker brokerWorker = workerFactory.getWorker(user);
                                    AtomicInteger qty = new AtomicInteger(1);
                                    user.getBniftyBuy925().getLotConfig().entrySet().forEach(day -> {
                                    ...
                                    try {
                                        ...
                                        user.getBniftyBuy925().straddleTradeMap.put(atmNiftyStrikeMap.getKey(), tradeData);
                                    //    mapTradeDataToSaveOpenTradeDataEntity(tradeData,true);
                                        ...
                                        LOGGER.info("Error while placing bnf nifty buy order: "+ atmNiftyStrikeMap.getKey()+":" + e.getMessage()+":"+e.code+":"+getAlgoName());
                                    ...
                                    LOGGER.info(new Gson().toJson(user.getBniftyBuy925().straddleTradeMap));
                                });
                            });
    ...
    @Scheduled(cron = "${bniftyBuy925.schedule.slMonitor}")
    public void sLMonitorScheduler() {
        //  LOGGER.info("short straddle SLMonitor scheduler started");

        userList.getUser().stream().filter(user -> user.getBniftyBuy925() != null && user.getBniftyBuy925().isNrmlEnabled()).forEach(user -> {
            BrokerWorker brokerWorker= workerFactory.getWorker(user);
            if (user.getBniftyBuy925().straddleTradeMap != null) {
                user.getBniftyBuy925().straddleTradeMap.entrySet().stream().filter(map -> map.getValue().isOrderPlaced && map.getValue().getEntryOrderId() != null)
                        .forEach(map -> {
                            try {
    ...
    @Scheduled(cron = "${bniftyBuy925.schedule.nextday.load}")
  ...
  //  @Scheduled(cron = "${bniftyBuy925.schedule.nextday.priceUpdate}")
    ...
    @Scheduled(cron = "${bniftyBuy925.schedule.nextday.slPlace}")
...
//                    LOGGER.info(price.doubleValue());
    ...
    @Scheduled(cron = "${bniftyBuy925.schedule.nextday.exit}")
                            ...
                            positions.stream().filter(position -> ("NRML".equals(position.product)||"MARGIN".equals(position.product)) && openTradeDataEntity.getStockName().equals(position.tradingSymbol) && (position.netQuantity != 0)).forEach(position -> {
                                ...
                                try {
                                    ...
                                    if (orderParams.transactionType.equals("SELL")) {
                                        ...
                                        sendMessage.sendToTelegram("Error while exiting order: " + orderParams.tradingsymbol + ": Exception: " + e.message + " order Input:" + new Gson().toJson(orderParams) + " positions: " + new Gson().toJson(position)+":"+getAlgoName(), telegramToken, "-646157933");
                                    }
                                    e.printStackTrace();
                                } catch (IOException e) {
    ...
    @Scheduled(cron = "${bniftyBuy925.schedule.nextday.slMonitor}")
