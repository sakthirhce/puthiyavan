...
public class WebSocketTicksSedaProcessor implements Processor {
    ...
    public void process(Exchange camelContextRoute) throws Exception {
        try {
            ...
            if (!ticks.isEmpty()) {
                exportTicksToFile(currentTime,dataExport,ticks,csvWriter,dateTimeFormatter);
                            ...
                            if (tradeData.getEntryOrderId() != null && tradeData.isOrderPlaced && !tradeData.isExited && !tradeData.isSLCancelled) {
                                ...
                                try {
                                    if (!tradeData.isWebsocketSlModified() && tradeData.isSlPriceCalculated && !tradeData.isSlPlaced) {
                                        ...
                                        if (lastTradedPrice < slPrice.doubleValue() && "BUY".equals(tradeData.getEntryType())) {
            ...
            try {
                LOGGER.error("error while processing tick:{},{}", camelContextRoute.getIn().getBody().toString(),e.getMessage());
            }catch (Exception e1){
    ...
    public void placeDynamicSellSL(TradeData tradeData, double lastTradedPrice, TradeStrategy strategy, BrokerWorker brokerWorker, User user, DateTimeFormatter dateTimeFormatter, LocalDateTime currentDateTime) {
        try {
    ...
    public void exportTicksToFile(LocalTime currentTime, boolean dataExport, List<Tick> ticks, CSVWriter csvWriter, DateTimeFormatter candleDateTimeFormat) {
        try {
            LocalTime tickStartTime = LocalTime.of(9, 14);
            LocalTime tickEndTime = LocalTime.of(15, 31);
            if (currentTime.isAfter(tickStartTime) && currentTime.isBefore(tickEndTime)) {
                if (dataExport) {
                    ticks.forEach(tick -> {
                        String[] dataHeader = {String.valueOf(tick.getInstrumentToken()), candleDateTimeFormat.format((TemporalAccessor) tick.getTickTimestamp()), String.valueOf(tick.getLastTradedPrice())};
                        csvWriter.writeNext(dataHeader);
                    });
                    csvWriter.flush();
                }
        ...
        try {
            ...
            if (globalTick != null) {
                ...
                if (tickData != null && !tickData.isEmpty()) {
                    double previousTick = tickData.get(tickData.size() - 1);
                    ...
                    if (diff > 0 && lastTradedPrice > 20) {
                        ...
                        double tenPercentOfPreviousTick = 0.15 * previousTick;
                        if (lastTradedPrice > percentageDiffBWPreviousAndRecentTick) {
                            ...
                            if ((lastTradedPrice + tenPercentOfPreviousTick) >= tradeData.getSlPrice().doubleValue()) {
                                tickMessage = tickMessage + "Another 10% to touch SL price. SLPrice:" + String.format("%,.2f", tradeData.getSlPrice());
                                LOGGER.info(tickMessage);
                                tradeSedaQueue.sendTelemgramSeda(tickMessage, "algo");
                                String freakSlMessage="placing sl based on freak tick:"+ tradeData.getStockName()+": last traded price"+lastTradedPrice;
    ...
    public void freakMoveInLast30Sec(Tick tick, double lastTradedPrice, TradeData tradeData, GlobalTickCache globalTickCache, Map<Long, Long> alertCount, TradeSedaQueue tradeSedaQueue) {
        try {
            ...
            globalTickCache.setHistoricData((int) tick.getInstrumentToken(), lastTradedPrice);
            ...
            LOGGER.error("An error occurred while processing the tick data", e);
        }
    }
