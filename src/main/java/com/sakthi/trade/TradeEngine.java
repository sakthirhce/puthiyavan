...
public class TradeEngine {
    ...
    public void loadStrategy() {
        ...
        tradeStrategyList.stream().forEach(strategy -> {
            String index = strategy.getIndex();
            if (strategyMap.containsKey(index) && strategy.getTradeDays().contains(dayFormat.format(date))) {
                AtomicInteger lotA=new AtomicInteger(0);
                config.stream().forEach(lot->{
                    String[] lotSplit=lot.split("-");
                    if(lotSplit[0].equals(index)){
                        lotA.getAndSet(Integer.valueOf(lotSplit[1]));
                    }
                ...
                if (indexTimeMap.containsKey(strategy.getEntryTime()) && strategies.size() > 0) {
                    ...
                    indexTimeMap.put(strategy.getEntryTime(), strategies);
                } else {
            ...
            strategyMap.entrySet().stream().forEach(indexEntry -> {
                ...
                if (!status.equals("error")) {
                    ...
                    try {
                        ...
                        List<TradeStrategy> strategies = timeStrategyListMap.get(hourMinFormat.format(date) + ":00");
                        if (strategies.size() > 0) {
                            strategies.stream().forEach(strategy -> {
                                ...
                                if (strategy.isRangeBreak()) {
                                    ...
                                    if(strategy.getRangeBreakTime().equals(hourMinFormat.format(date) + ":00")) {
                                        orbHighLow(historicalData.dataArrayList, orbHighLow, strategy, currentDateStr);
                                    }
                                        try {
                                            if(dateTimeFormat.parse(openDate+" "+hourMinFormat.format(date)).after(dateTimeFormat.parse(openDate+" "+hourMinFormat.format(strategy.getRangeBreakTime())))) {
                                                if (strategy.getRangeLow().doubleValue() > 0) {
                                                    if (hourMinFormat.parse(strategy.getRangeBreakTime()).before(hourMinFormat.parse(hourMinFormat.format(date) + ":00"))) {
                                    ...
                                    rangeStrikes = strikeSelection(currentDateStr, strategy, lastHistoricalData.close);
    ...
    public void orbHighLow(List<HistoricalData> orb, Map<String, Double> orbHighLow, TradeStrategy strategy, String currentDate) {
        ...
        String rangeStart = strategy.getEntryTime();
        String rangeEnd = strategy.getRangeBreakTime();
        for (HistoricalData candle : orb) {
            try {
                if (candleDateTimeFormat.parse(candle.timeStamp).after(candleDateTimeFormat.parse(currentDate + "'T'" + hourMinFormat.format(rangeStart) + ":00")) &&
                        candleDateTimeFormat.parse(candle.timeStamp).before(candleDateTimeFormat.parse(currentDate + "'T'" + hourMinFormat.format(rangeEnd) + ":00"))) {
                    if (candle.high > high) {
                        high = candle.high;
                    }
    ...
    public Map<Double, Map<String, StrikeData>> strikeSelection(String currentDate, TradeStrategy strategy, double close) {
        String strikeSelectionType = strategy.getStrikeSelectionType();
        String checkTime = strategy.getEntryTime();
        String index = strategy.getIndex();
